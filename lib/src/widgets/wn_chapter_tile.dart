...
class WnChapterTile extends StatelessWidget {
  const WnChapterTile({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final t = WnTheme.of(context).components.chapterTile;
...
///
/// ```dart
/// await WnUnlockSheet.show(context, chapter: ch, coinBalance: 120);
/// ```
class WnUnlockSheet {
  WnUnlockSheet._();
  ...
  static Future<bool?> show({
    ...
    return showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) {
        final cs = WnTheme.of(sheetContext).colorScheme;
        return SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  WnTheme.of(sheetContext).strings.lockedChapter,
                ...
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 10),
                    Text(
                      '${WnTheme.of(sheetContext).strings.balance}: ',
                      style: TextStyle(fontSize: 13.5, color: cs.textSecondary),
                    ),
                    Icon(
                ...
                FilledButton(
                  ...
                  child: Text(
                    '${WnTheme.of(sheetContext).strings.unlock} · ${chapter.coinCost} ${WnTheme.of(sheetContext).strings.coins}',
                  ),
                ),
                const SizedBox(height: 10),
                OutlinedButton(
                  onPressed: () => Navigator.of(sheetContext).pop(false),
                  child: Text(WnTheme.of(sheetContext).strings.maybeLater),
                ),
              ],
            ),
          ),
...
class WnCoinBadge extends StatelessWidget {
  const WnCoinBadge({super.key, required this.amount, this.dense = false});
