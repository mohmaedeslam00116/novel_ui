...
class WnPowerRankPodium extends StatelessWidget {
  const WnPowerRankPodium({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: WnDimens.screenPadding),
      padding: const EdgeInsets.all(WnDimens.lg),
      ...
      child: Column(
        children: [
          if (title != null)
            Padding(
              padding: const EdgeInsets.only(bottom: WnDimens.lg),
              child: Text(
                title!,
                style: TextStyle(
                  fontSize: 15,
...
class WnWaitOrPayTile extends StatefulWidget {
  const WnWaitOrPayTile({
    super.key,
  ...
  @override
  State<WnWaitOrPayTile> createState() => _WnWaitOrPayTileState();
}

class _WnWaitOrPayTileState extends State<WnWaitOrPayTile> {
  Timer? _timer;
...
class WnLatestUpdateTile extends StatelessWidget {
  const WnLatestUpdateTile({
    super.key,
    required this.book,
...
class WnReadingListPicker {
  WnReadingListPicker._();
...
/// hover/tooltips.
class WnAchievementRow extends StatelessWidget {
  const WnAchievementRow({super.key, required this.achievements});

  final List<WnAchievement> achievements;
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        separatorBuilder: (_, _) => const SizedBox(width: WnDimens.lg),
        itemBuilder: (context, i) {
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: a.locked
                        ...
                        : const LinearGradient(
                            ...
                            colors: WnColors.goldenTicketGradient,
                          ),
                    color: a.locked ? cs.surfaceLow : null,
                    border: Border.all(
                      color: a.locked ? cs.border : WnColors.primary,
                      width: 1.5,
                    ),
                  ),
                  child: Icon(
