...
import 'wn_badges.dart';
import 'wn_cover_view.dart';
import 'wn_status_badge.dart';

/// Compact vertical card used in library grids: cover, title, status badge.
///
/// ```dart
/// WnBookCard(book: book, onTap: () => openDetail(book))
/// ```
class WnBookCard extends StatelessWidget {
  ...
  ///
  /// Shorthand that returns a [WnBookListTile] so call sites stay uniform:
  /// ```dart
  /// WnBookCard.row(book: b, onTap: open)
  /// ```
  static Widget row({
    Key? key,
    required WnBook book,
    VoidCallback? onTap,
    Widget? trailing,
    double coverWidth = WnDimens.listCoverWidth,
  }) {
  ...
  Widget build(BuildContext context) {
    ...
    final t = WnTheme.of(context).components.bookCard;
    final effectiveWidth = width ?? t.coverWidth ?? WnDimens.gridCoverWidth;
    final effectiveRadius = t.coverRadius ?? WnDimens.coverRadius;
    return Semantics(
      button: onTap != null,
      container: true,
                ...
                children: [
                  ...
                  if ((showStatusBadge ?? t.showStatusBadge) ?? true)
                    Positioned(
                      top: 6,
                      left: 0,
                      child: WnStatusBadge(book.status),
                    ),
                  if ((updateCount ?? 0) > 0)
                    Positioned(
                      top: 2,
                      right: 2,
                      child: WnUpdateBadge(count: updateCount),
                    ),
                ],
              ),
              SizedBox(height: t.spacing ?? WnDimens.sm),
              Text(
                book.title,
                maxLines: 1,
...
class WnBookListTile extends StatelessWidget {
  const WnBookListTile({
    super.key,
    required this.book,
    this.onTap,
    this.trailing,
    this.coverWidth = WnDimens.listCoverWidth,
    this.maxTagLines = 1,
  });
  ...
  String _statusLabel(BuildContext context, WnBookStatus status) {
    final strings = WnTheme.of(context).strings;
    return switch (status) {
      WnBookStatus.ongoing => strings.ongoing,
      WnBookStatus.completed => strings.completed,
      WnBookStatus.hiatus => strings.hiatus,
    };
  }
}
