...
Future<void> showNovelReaderSettingsSheet({
...
class _ReaderSettingsSheetState extends State<_ReaderSettingsSheet> {
  late NovelReaderSettings _s = widget.settings;

  void _update(NovelReaderSettings s) {
    setState(() => _s = s);
    widget.onChanged(s);
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                for (final paper in NovelReaderPaper.values) ...[
                  Expanded(child: Center(child: _paperDot(paper))),
                  if (paper != NovelReaderPaper.values.last)
            ...
            if (_s.pageMode)
              SegmentedButton<NovelPageFlip>(
                segments: const [
                  ButtonSegment(value: NovelPageFlip.slide, label: Text('Slide')),
                  ButtonSegment(value: NovelPageFlip.cover, label: Text('Cover')),
                  ButtonSegment(
                    value: NovelPageFlip.instant,
                    label: Text('Instant'),
                  ),
                ],
  ...
  Widget _paperDot(NovelReaderPaper paper) {
    final colors = kNovelReaderPapers[paper]!;
    final selected = _s.paper == paper;
    final cs = NovelTheme.of(context).colorScheme;
    final labels = {
      NovelReaderPaper.light: 'White',
      NovelReaderPaper.sepia: 'Parchment',
      NovelReaderPaper.green: 'Mint',
      NovelReaderPaper.gray: 'Gray',
      NovelReaderPaper.kraft: 'Kraft',
      NovelReaderPaper.night: 'Night',
    };
