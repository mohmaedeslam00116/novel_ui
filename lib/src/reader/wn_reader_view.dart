...
import 'wn_reader_settings_sheet.dart';
import 'wn_text_paginator.dart';
...
/// )
/// ```
class WnReaderView extends StatefulWidget {
  const WnReaderView({
    super.key,
    required this.chapter,
    this.totalChapters = 1,
    this.settings = const WnReaderSettings(),
  ...
  final WnChapter chapter;
  final int totalChapters;
  final WnReaderSettings settings;
  final ValueChanged<WnReaderSettings>? onSettingsChanged;
  ...
  /// [WnParagraphCommentsSheet.show] here, or your own screen.
  final ValueChanged<int>? onParagraphComments;

  @override
  State<WnReaderView> createState() => _WnReaderViewState();
}

class _WnReaderViewState extends State<WnReaderView> {
  bool _chromeVisible = false;
  Timer? _autoScrollTimer;
  late WnReaderSettings _settings = widget.settings;
  final ScrollController _scrollController = ScrollController();
  PageController? _pageController;
  List<WnReaderPage>? _pages;
  double _progress = 0;
  int _pageIndex = 0;
  ...
  @override
  void didUpdateWidget(covariant WnReaderView oldWidget) {
  ...
  void _syncAutoScroll(WnReaderSettings settings) {
  ...
  void _applySettings(WnReaderSettings s) {
  ...
  void _openCatalogFallback() {
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) {
        final strings = WnTheme.maybeOf(sheetContext)?.strings;
        return SafeArea(
          child: Column(
  ...
  Widget build(BuildContext context) {
    final colors = kWnReaderPapers[_settings.paper]!;
          ...
          Positioned.fill(
            child: Padding(
              padding: const EdgeInsets.symmetric(
                horizontal: WnDimens.readerPageInsetH,
              ),
  ...
  // ── Scroll mode ────────────────────────────────────────────────────────
  Widget _buildScroll(WnReaderColors colors) {
    ...
    return ListView.builder(
      ...
      padding: EdgeInsets.fromLTRB(
        0,
        mediaQuery.padding.top + WnDimens.readerPageInsetV,
        0,
        mediaQuery.padding.bottom + WnDimens.readerPageInsetV + 24,
      ),
  ...
  Widget _chapterHeader(WnReaderColors colors) {
    return Padding(
  ...
  Widget _chapterFooter(WnReaderColors colors) {
    final strings = WnTheme.maybeOf(context)?.strings;
  ...
  // ── Page mode ──────────────────────────────────────────────────────────
  Widget _buildPages(WnReaderColors colors) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (_pages == null || _computedPagesHash != pagesKey) {
          ...
          _pages = WnTextPaginator.paginate(
  ...
  void _goToRelativePage(int delta) {
    ...
    if (target < 0) {
      ...
      widget.onNextChapter?.call();
    } else if (_settings.pageFlip == WnPageFlip.instant ||
        MediaQuery.of(context).disableAnimations) {
      controller.jumpToPage(target);
    } else {
      controller.animateToPage(
        target,
        duration: WnDimens.fast * 2,
  ...
  // ── Chrome ─────────────────────────────────────────────────────────────
  List<Widget> _chrome(BuildContext context, WnReaderColors colors) {
  ...
  /// Resolves [WnReaderSettings.backImage]: http(s) URLs render from the
  ...
  bool get _isNightPaper =>
      _settings.paper == WnReaderPaper.night ||
      _settings.paper == WnReaderPaper.gray;
}
...
class _ParagraphText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cs = WnTheme.maybeOf(context)?.colorScheme;
...
/// Applies the configured [WnPageFlip] turn animation to one page of the
...
class _PageFlipEffect extends StatelessWidget {
  ...
  final int index;
  final WnPageFlip style;
  final Widget child;

  @override
  Widget build(BuildContext context) {
    if (style != WnPageFlip.cover ||
        controller == null ||
