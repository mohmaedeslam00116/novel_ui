...
class NovelReaderSegment {
  const NovelReaderSegment({required this.paragraphIndex, required this.text});
...
class NovelReaderPage {
  const NovelReaderPage(this.segments);

  final List<NovelReaderSegment> segments;
}
...
abstract final class NovelTextPaginator {
  static List<NovelReaderPage> paginate({
    ...
    final pages = <NovelReaderPage>[];

    var currentPage = <NovelReaderSegment>[];
    var usedHeight = 0.0;
    var lastParagraphInPage = -1;

    void flushPage() {
      if (currentPage.isNotEmpty) {
        pages.add(NovelReaderPage(List.of(currentPage)));
        currentPage = <NovelReaderSegment>[];
        usedHeight = 0;
    ...
    for (var p = 0; p < paragraphs.length; p++) {
      ...
      if (remaining.trim().isEmpty) {
        ...
        currentPage.add(NovelReaderSegment(paragraphIndex: p, text: ''));
        usedHeight += gap;
        continue;
      }
      ...
      while (remaining.isNotEmpty) {
        ...
        if (tp.height <= available) {
          currentPage.add(NovelReaderSegment(paragraphIndex: p, text: remaining));
        ...
        currentPage.add(NovelReaderSegment(paragraphIndex: p, text: head));
